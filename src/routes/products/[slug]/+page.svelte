...
>
	<div class="aspect-square w-full overflow-hidden rounded-lg bg-gray-200">
		<img
			...
			src={products[data.productId - 1].imageSrc}
			alt={products[data.productId - 1].name}
		/>
	</div>
	<div>
		<div class="mt-4 break-all">
			<h3 class="text-slate-900 text-2xl font-bold">{products[data.productId - 1].name}</h3>
			<div class="ml-2">
				<p class="text-sm text-slate-800 mb-2">{products[data.productId - 1].author}</p>
		...
		<div>
			<p class="font-bold text-slate-900 text-xl mt-4 ml-2">
				{products[data.productId - 1].price} Ft
			</p>

...
<section class="w-full max-w-7xl px-4">
	<h2 class="mb-6 text-2xl font-bold">Vásárlási előzmények</h2>

	{#if purchases.length === 0}
		<p class="py-8 text-center text-gray-500">Nem érhetők el vásárlási előzmények</p>
	{:else}
			...
			{#each purchases as purchase}
				<div class="h-full rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
					<div class="mb-4 flex items-center justify-between">
						<div>
							<p class="text-sm text-gray-500">Rendelés #{purchase.id}</p>
						...
						{#each purchase.books as item}
							<div class="flex items-center justify-between py-2">
								<div>
									...
									<p class="text-sm text-gray-500">Mennyiség: {item.quantity}</p>
								</div>
					...
					<div class="mt-auto flex items-center justify-between border-gray-200 pt-4">
						<p class="font-medium">Összesen</p>
